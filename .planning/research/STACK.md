# Stack Research — Hype On Media Website Platform

**Date:** 2026-03-21
**Context:** Greenfield Next.js agency website. Marketing landing page + MDX blog + authenticated client portal. Deployed on DigitalOcean with nginx + PM2 (not Vercel). Must coexist with Orbit on the same server.
...
content/
  blog/
    youtube-revenue-2026.mdx
    audit-framework.mdx
    ...
velite.config.ts          # schema + collections
```

Velite outputs typed JSON at build time. Import with `import { posts } from '#content'`. Zero runtime cost.
...
server: cd /opt/hypeon-website && git pull && npm ci && npm run build && pm2 restart hypeon-website
```

Automate with a `deploy.sh` script or GitHub Actions later. Manual is fine for Phase 1.
...
```bash
npm install next@latest react@latest react-dom@latest sharp
npm install motion
...
This is deliberately lean. Every package has a clear job. No "nice to have" dependencies.
...
*Research completed 2026-03-21. Versions verified via npm registry and official release blogs.*
